...
(() => {
  ...
  const getBotRandomFigure = (arr) => {
    ...
    if (arr[random] === 'камень' || arr[random] === 'rock') {
      botFigureNum = 1;
    }
  ...
  const getUserFigure = (lang = 'RUS') => {
    ...
    if (lang === 'ENG') {
      ...
      // !!! if EXIT
      if (choice === null) {
        const exitE = confirm(`Are you sure you don't want to play?`);
      ...
      if (choice != null) {
        ...
        // * invalid input
        if (choice[0] != 's' && choice[0] != 'p' && choice[0] != 'r') {
          alert(`
          Invalid input! 
          You entered: ${choice},
          But you need: 'rock' or 'scissors' or 'paper'`);
          // getGame(lang);
